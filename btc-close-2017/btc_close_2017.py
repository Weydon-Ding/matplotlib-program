...
with open('收盘价Dashboard.html', 'w', encoding='utf-8') as html_file:
	content = '<html><head><title>收盘价Dashboard</title><meta charset="utf-8"></head><body>\n'
	html_file.write(content)
	for svg in ['收盘价折线图 (￥).svg', '收盘价对数变换折线图 (￥).svg',
		'收盘价月日均值 (￥).svg', '收盘价周日均值 (￥).svg',
		'收盘价星期均值 (￥).svg']:
			content = '	<object type="image/svg+xml" data="{0}" height=500></object>\n'
			html_file.write(content.format(svg))
